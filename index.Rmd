...
I put this in a separate file because it is quite a slow process. See calculate_sr RMarkdown file [here](calculate_sr.html). 
...
1. Christin: 
    ...
    - double check that the age-sex classes have all been assigned correctly and ready to go 
    - missing date/time/position removed?
    - think more on the overlap criteria for being alive in the same age-sex class and if 1 day (instead of 365 days) is most appropriate metric, especially for juveniles and lactating females

2. Will:

    - ~~change 35 sample size restriction to be just at individual egno level (not at age-sex level)~~
    - ~~change the overlap criteria to just if they are both available for at least one day (rather than 365) and think more on that~~
    - ~~put in a filter to explicitly kick out any sightings of dead whales (both a behavior code of 'DEAD' OR an assumption of death if not seen for 6 consecutive years)~~
    - ~~put in a filter to kick out missing latitude/longitude values~~
    - ~~move over the R code to remove duplicate rows (between sections 6.1 and 6.2)~~
    - move over the R code to run social differentiation test (but maybe not... see email from Christin 3/20/2019)
