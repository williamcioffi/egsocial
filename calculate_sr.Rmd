...
- Only animals with at least 35 total sightings were considered.
    -  I implemented sighting requirement in a very conservative way.
...
-  If either individual in the dyad wasn't sighted more than `numsampcutoff` during the time they were both alive let's make their association index NA. **This is quite restrictive and probably not neccessary. Especially since 35 is fairly high to begin with. We could apply the `numsampcutoff` to total observations not accounting for overlap if we wanted to be slightly less restrictive.**
